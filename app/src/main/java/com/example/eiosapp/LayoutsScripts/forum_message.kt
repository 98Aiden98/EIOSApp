...
class forum_message : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val disciplineid = intent.getStringExtra("disciplineidForumMessage")
        val title = intent.getStringExtra("titleForumMessage")
    ...
    fun scrollDown() {
        val scrollThread: Thread = object : Thread() {
            override fun run() {
                try {
                    sleep(200)
                    this@forum_message.runOnUiThread(Runnable { scrollView.fullScroll(View.FOCUS_DOWN) })
    ...
    private fun addSendMessage(MainSection: LinearLayout,MainLayout: LinearLayout, forumMessage: ForumMessage, disciplineid: Int)
    {
        ...
        ControlDotSection.setBackgroundResource(R.drawable.rating_plan_ball_style)
        ...
        val rawtitle = forumMessage.text // Заголовок темы
        ...
        nsSection.setBackgroundResource(R.drawable.rating_plan_ball_style)
        ...
        messageDataTextView.setTextColor(Color.WHITE)
    ...
    private fun createForumMessage(MainSection: LinearLayout, disciplineid: Int,MainLayout: LinearLayout,forumMessage: List<ForumMessage>)
    {
        ...
        for(i in (0..<forumMessage.size).reversed()) {
            ...
            textSection.orientation = LinearLayout.VERTICAL
            textSection.setPadding(10,10,10,10)
            val rawtitle = forumMessage[i].text // Заголовок темы
            val titleTextView = TextView(this)
            titleTextView.setTextColor(Color.WHITE)
            titleTextView.text = rawtitle
            textSection.addView(titleTextView)
